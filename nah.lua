...
function FastAttack()
    ...
    local reShootGunEvent = net:WaitForChild("RE/ShootGunEvent")
    local reRegisterHit = net:WaitForChild("RE/RegisterHit")
    
    local combatController = replicatedStorage:WaitForChild("Controllers"):WaitForChild("CombatController")
    local successShoot, shootFunction = pcall(function()
        return getupvalue(require(combatController).Attack, 9)
    end)
    ...
    local function getValidator()
        if not successShoot or not shootFunction then return end
    ...
    local function getBladeHits(character, distance)
        ...
        local characters = game:GetService("Players"):GetCharacters()
        local hitboxLimbs = config.HitboxLimbs
        
        local function process(folder, canAttack)
            for _, enemy in pairs(folder:GetChildren()) do
                ...
                if basePart and isAlive(enemy) and (position - basePart.Position).Magnitude <= (distance or config.Distance) then
                    if folder == characters then
                        local enemyPlayer = game:GetService("Players"):GetPlayerFromCharacter(enemy)
                        if enemyPlayer and not CheckPlayerAlly(enemyPlayer) then
        ...
        if config.AttackMobs then process(enemies, true) end
        if config.AttackPlayers then process(characters, false) end
        
        return bladeHits
    end
    
    local function getClosestEnemy(character, distance)
        local bladeHits = getBladeHits(character, distance)
        local closest, minDistance = nil, math.huge
        for _, hit in pairs(bladeHits) do
            local magnitude = (hit[2].Position - character:GetPivot().Position).Magnitude
    ...
    local function getGunHits(character, distance)
        local bladeHits = getBladeHits(character, distance)
        local gunHits = {}
        for _, hit in pairs(bladeHits) do
            if #gunHits == 0 or (hit[2].Position - gunHits[1].Position).Magnitude <= 10 then
                table.insert(gunHits, hit[2])
            end
        end
        return gunHits
    end
    ...
    local function getCombo()
        local combo = tick() - config.ComboDebounce <= 0.4 and config.M1Combo or 0
        combo = combo >= 4 and 1 or combo + 1
        config.ComboDebounce = tick()
        config.M1Combo = combo
        return combo
    end
    
    local function expandHitBox(bladeHits)
        for _, hit in pairs(bladeHits) do
            hit[2].Size = Vector3.new(50, 50, 50)
            hit[2].Transparency = 1
        end
    end
    
    local function checkStun(character, toolTip)
        local humanoid = character:FindFirstChild("Humanoid")
        local stun = character:FindFirstChild("Stun")
        return not (humanoid.Sit and (toolTip == "Sword" or toolTip == "Melee" or toolTip == "Gun")) and (not stun or stun.Value <= 0)
    end
    
    local function shootSkullGuitar(equipped, position)
        equipped.RemoteEvent:FireServer("TAP", position)
    end
    
    local function useGunShoot(character, equipped)
        if not equipped.Enabled then return end
        local shootType = config.SpecialShoots[equipped.Name] or "Normal"
        
        if shootType == "Normal" then
            local hits = getGunHits(character, 120)
            local target = hits[1] and hits[1].Position
            if target then
                equipped:SetAttribute("LocalTotalShots", (equipped:GetAttribute("LocalTotalShots") or 0) + 1)
                local validator = getValidator()
                if validator then GunValidator:FireServer(validator) end
                for _ = 1, config.ShootsPerTarget[equipped.Name] or 1 do
                    reShootGunEvent:FireServer(target, hits)
                end
            end
        elseif shootType == "Overheat" then
            local data = config.Overheat[equipped.Name]
            if data.Shooting then return end
            local target = getClosestEnemy(character, data.Distance)
            if target then
                data.Shooting = true
                while equipped.Parent == character and data.TotalOverheat < data.MaxOverheat do
                    if target.Parent and isAlive(target.Parent) then
                        equipped:SetAttribute("LocalTotalShots", (equipped:GetAttribute("LocalTotalShots") or 0) + 1)
                        local validator = getValidator()
                        if validator then GunValidator:FireServer(validator) end
                        reShootGunEvent:FireServer(target.Position, {target})
        ...
        elseif shootType == "Position" or (shootType == "TAP" and equipped:FindFirstChild("RemoteEvent")) then
            local target = getClosestEnemy(character, 200)
            if target then
                if equipped.Name == "Skull Guitar" then
                    shootSkullGuitar(equipped, target.Position)
                else
                    equipped:SetAttribute("LocalTotalShots", (equipped:GetAttribute("LocalTotalShots") or 0) + 1)
                    local validator = getValidator()
                    if validator then GunValidator:FireServer(validator) end
                    if shootType == "TAP" then
                        ...
                        reShootGunEvent:FireServer(target.Position)
                    end
                end
            end
        end
    ...
    local function useNormalClick(character, cooldown)
        local bladeHits = getBlade MilitHits(character)
        local enemyHitBox = bladeHits[1] and bladeHits[1][2]
        if enemyHitBox then
            local successFlags = pcall(function()
                return require(game:GetService("ReplicatedStorage").Modules.Flags).COMBAT_REMOTE_THREAD
            end)
            if successFlags and hitFunction then
                reRegisterAttack:FireServer(cooldown)
                hitFunction(enemyHitBox, bladeHits)
            else
                reRegisterAttack:FireServer(cooldown)
                reRegisterHit:FireServer(enemyHitBox, bladeHits)
            end
    ...
    local function attack()
        if not Settings.AutoClick or tick() - Module.AttackCooldown <= 1 or not isAlive(player.Character) then return end
        ...
        if not equipped or not (toolTip == "Gun" or toolTip == "Melee" or toolTip == "Blox Fruit" or toolTip == "Sword") then return end
        
        local cooldown = equipped:FindFirstChild("Cooldown") and equipped.Cooldown.Value or 0.3
        if tick() - config.Debounce < cooldown or not checkStun(character, toolTip) then return end
        
        local combo = getCombo()
        cooldown = combo >= 4 and cooldown + 0.05 or cooldown
        config.Debounce = combo >= 4 and toolTip ~= "Gun" and tick() + 0.05 or tick()
        
        if toolTip == "Blox Fruit" then
            if toolName == "Ice-Ice" or toolName == "Light-Light" then
                useNormalClick(character, cooldown)
            elseif equipped:FindFirstChild("LeftClickRemote") then
                useFruitM1(character, equipped, combo)
            end
        elseif toolTip == "Gun" and successShoot and shootFunction and Settings.AutoShoot then
            useGunShoot(character, equipped)
        else
            useNormalClick(character, cooldown)
        end
    end
    
    runService.Stepped:Connect(attack)
end
            ...
            for _, enemy in ipairs(enemies) do
                ...
                if getgenv().BringMonster and enemy:FindFirstChild("Humanoid") and enemy:FindFirstChild("HumanoidRootPart") then
                    ...
                    if humanoid and rootPart and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        if getgenv().StartMagnet and (enemy.Name == MonFarm or enemy.Name == Mon) and humanoid.Health > 0 and distance <= 500 then
                            ...
                            elseif (enemy.Name == MonFarm or enemy.Name == Mon) and PosMon and (rootPart.Position - PosMon.Position).Magnitude <= 5000 then
                                if rootPart.Parent then
                                    rootPart.CanCollide = false
                                    rootPart.Size = Vector3.new(60, 60, 60)
                                    rootPart.CFrame = PosMon
...
Main:AddParagraph({
     Title = "Tyrant (Not Work BTW)",
     Content = "Iam Lazy To Combine All"
})

Toggle = Main:AddToggle("Toggle", { Title = "Fully Summon And Attack Tyrant", Default = false })
Toggle:OnChanged(function(Value)
    getgenv().FullyTyrant = Value
    StopTween(getgenv().FullyTyrant)
end)
...
game:GetService("RunService").Heartbeat:Connect(function()
    ...
    local Tyrant = {
        ...
        ["Island Boy"] = CFrame.new(-16901.26171875, 84.06756591796875, -192.88906860351562),
    ...
    pcall(function()
        if getgenv().TyrantBring then
            for _, v in pairs(workspace.Enemies:GetChildren()) do
                if Tyrant[v.Name] and v:FindFirstChild("HumanoidRootPart") then
                    v.HumanoidRootPart.CFrame = Tyrant[v.Name]
                    v.Head.CanCollide = false
                    v.Humanoid.Sit = false
                    v.Humanoid:ChangeState(11)
                    task.wait(0.1)
            ...
            if workspace.Enemies:FindFirstChild("Tyrant of the Skies") then
                for _, v in pairs(workspace.Enemies:GetChildren()) do
                    if v.Name == "Tyrant of the Skies" and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                        ...
                        repeat task.wait() until not _G.FarmDaiBan or not v.Parent or v.Humanoid.Health <= 0
                        task.wait(1)
                    end
                end
            elseif allEyesActive or islandModel.Eye4.Transparency == 0 then
                for _, model in pairs(islandModel:GetChildren()) do
                    if model:FindFirstChild("EagleBossArena") then
                        for _, v in pairs(model.EagleBossArena:GetChildren()) do
                            if v.Name == "Tree" then
                                topos(CFrame.new(v.WorldPivot.Position))
                            end
                        end
                    end
                end
                ...
                if foundMob then
                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                        if (v.Name == "Isle Outlaw" or v.Name == "Island Boy" or v.Name == "Isle Champion" or v.Name == "Serpent Hunter" or v.Name == "Skull Slayer") and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                            ...
                            repeat task.wait() until not getgenv().FullyTyrant or not v.Parent or v.Humanoid.Health <= 0 or workspace.Map.CakeLoaf.BigMirror.Other.Transparency == 0 or workspace.Enemies:FindFirstChild("Tyrant of the Skies [Lv. 2600] [Raid Boss]")
                        end
                    end
                elseif  BypassTP and (player.Character.HumanoidRootPart.Position - TyrantoftheSkies.Position).Magnitude > 1500 then
...
TimeRaidParagraph = Blog:AddParagraph({
    Title = "Time Raid:",
    Content = ""
})
spawn(function()
...
Toggle = Blog:AddToggle("Toggle", {Title = "Start Raid", Default = false})
Toggle:OnChanged(function(Value)
    getgenv().StartRaid = Value
end)
spawn(function()
    while wait() do
        pcall(function()
            if getgenv().StartRaid then
...
Toggle = Race:AddToggle("Toggle", {Title = "Auto Draco Trails (Beta)", Default = false})
Toggle:OnChanged(function(Value)
    getgenv().AutoDracoTrailsBeta = Value
end)
local Players = game:GetService("Players")
local Workspace = game:GetService("Workspace")
...
local player = Players.LocalPlayer
getgenv().AutoDracoTrailsBeta = true

local function AutoDracoTrial()
    local function TweenTo(targetCFrame, duration)
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            local hrp = player.Character.HumanoidRootPart
            local tweenInfo = TweenInfo.new(duration or 3, Enum.EasingStyle.Linear)
            local tween = TweenService:Create(hrp, tweenInfo, {CFrame = targetCFrame})
            tween:Play()
            repeat
                task.wait(2)
                if not getgenv().AutoDracoTrailsBeta then 
                    tween:Cancel()
                    return
                end
            until (hrp.Position - targetCFrame.Position).Magnitude < 5
            tween:Cancel()
    ...
    local function InteractWithPrompt()
        ...
        if hrp then
            for _, v in pairs(Workspace:GetDescendants()) do
                if v:IsA("ClickDetector") and (v.Parent.Position - hrp.Position).Magnitude < 20 then
                    fireclickdetector(v)
                    break
                end
            end
    ...
    task.spawn(function()
        ...
        while task.wait(0.5) do
            if getgenv().TrialTeleportDraco and trialPart and trialPart:IsA("Part") then
                ...
                if getgenv().AutoDracoTrailsBeta then
                    pcall(function()
                        local character = player.Character or player.CharacterAdded:Wait()
                        ...
                        if trialTeleport and trialTeleport:IsA("Part") then
                            TweenTo(CFrame.new(trialTeleport.Position), 3)
                            local distance = (hrp.Position - trialTeleport.Position).Magnitude
                            if distance < 5 then
                                ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("DracoTrial"):InvokeServer()
                            end
                        end

                        local sequence = {
                            {cf = CFrame.new(-40184.3359, 9325.25098, 23261.6094), time = 3},
                            {cf = CFrame.new(-40511.5195, 9374.39648, 23461.0664), time = 3},
                            {cf = CFrame.new(-39928.0703, 10683.3232, 22997.6172), time = 3},
                            {cf = CFrame.new(-40043.7031, 9374.39648, 22793.7266), time = 3},
                            {cf = CFrame.new(-39916.7891, 10683.3535, 22998.5039), time = 3},
                            {cf = CFrame.new(-39608.3047, 9374.39746, 23470.6895), time = 6}
                        }
                        ...
                        for _, step in ipairs(sequence) do
                            ...
                            TweenTo(step.cf, step.time)
                            task.wait(5)
                        end

                        if getgenv().AutoDracoTrailsBeta then
                            local extraCFrame = CFrame.new(-39908.7578, 10690.8496, 22986.3145, 0.997909248, 0.00107694988, -0.0646221414, -0.00116624858, 0.999998391, -0.00134415214, 0.0646205917, 0.00141670729, 0.99790889)
                            TweenTo(extraCFrame, 4)
                            task.wait(5)

                            local finalCFrame = CFrame.new(-39829.8164, 10669.5479, 23009.123)
                            TweenTo(finalCFrame, 4)
                            task.wait(2)
...
local settings = { AttackRange = 100, AttackDelay = 0.001 }
local worldOrigin = Workspace:WaitForChild("_WorldOrigin")
local eagleTapEffect = worldOrigin:FindFirstChild("FalconTAPEffect") or worldOrigin:FindFirstChild("GravityTAPEffect")
...
local function getEnemiesInRange(range)
    ...
    for _, enemy in ipairs(Workspace.Enemies:GetChildren()) do
        ...
        if enemyHRP and isEntityAlive(enemy) and (enemyHRP.Position - playerPos).Magnitude <= range then
            table.insert(targets, { enemy, enemyHRP })
        end
    end
    return targets
end

local function falconTapEffect(targetHRP)
    ...
    eagleTapEffect:FireServer(effectData)
end
...
local function attackNoCooldown()
    ...
    local enemiesInRange = getEnemiesInRange(settings.AttackRange)
    if equippedWeapon:FindFirstChild("LeftClickRemote") then
        for _, enemyData in ipairs(enemiesInRange) do
            local direction = (enemyData[2].Position - HRP.Position).Unit
            task.defer(function()
                pcall(function()
                    equippedWeapon.LeftClickRemote:FireServer(direction, 1)
                    falconTapEffect(enemyData[2])
                end)
            end)
        end
    elseif #enemiesInRange > 0 then
        local mainTarget = enemiesInRange[1][2]
        task.defer(function()
            pcall(function()
                attackEvent:FireServer(0)
                hitEvent:FireServer(mainTarget, enemiesInRange)
                falconTapEffect(mainTarget)
            end)
        end)
    end
